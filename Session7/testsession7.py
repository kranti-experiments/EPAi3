...
def test_function_docstringlen():
    ...
    doclensanity = session7.checkdoclen()
    ...
    #Test-5: Checking if the session7 functions have enough documentation or not
    assert doclensanity(session7.checkdoclen) == True, "Function does not have enough documentation!" 
    assert doclensanity(session7.fibgen) == True, "Function does not have enough documentation!" 
    assert doclensanity(session7.globalcounter) == True, "Function does not have enough documentation!" 
    assert doclensanity(session7.funccounter) == True, "Function does not have enough documentation!" 

def test_fibonacci_number():
    fibonacci = session7.fibgen()
...
def test_global_dict_counts():
    ...
    add_func = session7.globalcounter(add)
    add_counts = 3
    for index in range(add_counts):
        add_func(index, index+1)
    assert session7.countfunccalls['add'] == add_counts, 'Add Function Calls Count not Matched!'
    #Test-2: Checking the count is incremented even if the function is called with same arguments as previous
    add_func(index, index+1)
    assert session7.countfunccalls['add'] == (add_counts+1), 'Add Function Calls Count not Matched!'

    #Test-3: Checking the count is not incremented without function call
    assert 'mul' not in session7.countfunccalls, 'Mul Function Count incremented without a single call!'
    #Test-4: Checking if the count is incremented by 1 or uses previous calls of 'add'
    mul_func = session7.globalcounter(mul)
    mul_func(1, 2)
    assert session7.countfunccalls['mul'] == 1, 'Mul Function Calls Count not Matched!'
    ...
    assert 'add' in session7.countfunccalls, 'Add Function missing in the dictionary!'
    assert session7.countfunccalls['add'] == (add_counts+1), 'Add Function Calls Count not Matched!'
    assert session7.countfunccalls['mul'] == (mul_counts+1), 'Mul Function Calls Count not Matched!'

    #Test-6: Checking whether the count is incremented or not in valueerror scenario
    div_func = session7.globalcounter(div)
    with pytest.raises(ValueError):
        div_func(1, 0)
        assert session7.countfunccalls['div'] == 1, 'Div Function Calls Count not Matched!'
    ...
    assert session7.countfunccalls['div'] != div_counts, 'Div Function Calls Count Should not be Matched!'


def test_ind_dict_counts():
    #Test-1: Checking the number of calls to add()
    add_dict = dict()
    add_func = session7.funccounter(add, add_dict)
    add_counts = 3
    for index in range(add_counts):
        add_func(index, index+1)
    ...
    #Test-4: Checking if the count is incremented by 1 or uses previous calls of 'add'
    mul_func = session7.funccounter(mul, mul_dict)
    mul_func(1, 2)
    ...
    #Test-6: Checking whether the count is incremented or not in valueerror scenario|
    div_dict = dict()
    div_func = session7.funccounter(div, div_dict)
    with pytest.raises(ValueError):
        div_func(1, 0)
